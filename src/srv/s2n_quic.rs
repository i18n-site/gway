// https://raw.githubusercontent.com/aws/s2n-quic/refs/heads/main/quic/s2n-quic-h3/src/s2n_quic.rs
  ...
  fn poll_accept_recv(
    ...
    let recv = match ready!(self.recv_acceptor.poll_accept_receive_stream(cx)) {
      ...
      Ok(None) => {
        ...
        return Poll::Ready(Err(ConnectionErrorIncoming::ApplicationClose {
          error_code: Code::H3_NO_ERROR.into(),
        }));
      }
      Err(e) => {
...
fn convert_stream_error_to_h3_stream_error_incoming(
  ...
  match error {
    s2n_quic::stream::Error::StreamReset { error, .. } => StreamErrorIncoming::StreamTerminated {
      error_code: error.into(),
    },
